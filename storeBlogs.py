...
def initializeDatabase():
	...
	db.execute('''CREATE TABLE IF NOT EXISTS entities (entry_author text, entry_title text,
				 	 entry_date date, entry_stamp timestamp, entry_delta int, 
				 	 entry_link text unique, entityCount int, entityName text, 
				 	 entityType text)''')

	#create table for places mentioned
	db.execute('''CREATE TABLE IF NOT EXISTS places (entry_author text, entry_title text,
				 	 entry_date date, entry_stamp timestamp, entry_delta int, 
